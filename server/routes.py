...
@app.route("/cameras")
def cameras():
    # Get all cameras in camera net (which are guaranteed unique by id)
    cameras = Camera.query.all()
    return render_template('cameras.html', cameras=cameras)

@app.route("/images/<id>")
def images(id):
    # Get camera by ID (which we will use to get all images for that camera)
    camera = Camera.query.filter_by(id=id).first()
    image_paths = [f'/static/images/{camera.name}_{camera.id}/{image.id}.jpg' for image in camera.images]
    return render_template('image.html', image_paths=image_paths)
...
def log_image():
    if request.method == 'POST':
        ...
        # NOTE: can use camera_node=camera OR camera_id=int(metadata['id'])
        ...
        image.path = os.path.join('server', 'static', 'images', f'{camera.name}_{camera.id}', f'{image.id}.jpg')
        db.session.commit()
        ...
        num_images_to_keep_in_database = 10
...
# This route is really only kept as an example for how we might decode/retrieve the image
